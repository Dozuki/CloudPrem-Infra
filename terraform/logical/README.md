...
| <a name="input_operator_primary_site"></a> [operator\_primary\_site](#input\_operator\_primary\_site) | Sitename at siteid 2: the install's base site. Every install has exactly one, and it is<br/>either "onprem" (standard CloudPrem) or the platform's own base sitename (shared GovCloud is<br/>"dozuki"). siteid 1 is the internal "base" template site and is never this.<br/><br/>The operator applies every entitled FeatureRequest to this site plus each Subsite CR. The<br/>operator chart hardcodes "onprem", so any install whose base site is named otherwise must<br/>set this or every reconcile fails with "Site '<name>' not found", permanently.<br/><br/>Confirm with: SELECT name FROM sites.site\_index WHERE siteid = 2; | `string` | `"onprem"` | no |
