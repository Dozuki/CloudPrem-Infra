...
| [kubernetes_secret_v1.grafana_common_config](https://registry.terraform.io/providers/hashicorp/kubernetes/latest/docs/resources/secret_v1) | resource |
...
| <a name="input_customer_tls_externally_managed"></a> [customer\_tls\_externally\_managed](#input\_customer\_tls\_externally\_managed) | Set true when the customer provides their own TLS cert (e.g. their wildcard) and it's stored in Vault.<br>When true, an ExternalSecret is created that syncs the cert+key from Vault path<br>secret/{customer}/{environment}/tls (keys: cert, key) into the K8s Secret named tls-secret.<br>The chart's tls.externallyManaged flag is also set so it skips rendering tls-secret and<br>drops the cert-manager Gateway annotation.<br>The cert/key must be pre-seeded in Vault out-of-band (terraform does not touch the cert content). | `bool` | `false` | no |
