...
| [external_external.alertmanager_slack_silence_htpasswd_hash](https://registry.terraform.io/providers/hashicorp/external/latest/docs/data-sources/external) | data source |
...
| <a name="input_alertmanager_slack_silence_password"></a> [alertmanager\_slack\_silence\_password](#input\_alertmanager\_slack\_silence\_password) | Shared password used by the standalone Slack interaction handler to create Alertmanager silences. When set on AWS, Terraform publishes only its htpasswd hash to the environment's Vault ops-auth secret and enables the interactive button. | `string` | `""` | no |
...
| <a name="input_dms_task_arn"></a> [dms\_task\_arn](#input\_dms\_task\_arn) | If BI is enabled, the DMS Serverless replication config arn. Not consumed by this layer; kept for the contract and for operator use. | `string` | n/a | yes |
