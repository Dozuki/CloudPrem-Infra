...
| <a name="input_app_image_flavor"></a> [app\_image\_flavor](#input\_app\_image\_flavor) | App image family: legacy (vm-in-a-can app repo) or slim (monolith-app family). Slim switches the chart's images.app.path to monolith-app and enables the dedicated beanstalkd image. | `string` | `"legacy"` | no |
...
| <a name="input_beanstalkd_tag"></a> [beanstalkd\_tag](#input\_beanstalkd\_tag) | Tag for the dedicated beanstalkd fork image (slim flavor only; repo <registry>/beanstalkd). | `string` | `""` | no |
