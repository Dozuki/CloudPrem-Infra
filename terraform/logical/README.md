...
| <a name="input_nextjs_service_jwt_enabled"></a> [nextjs\_service\_jwt\_enabled](#input\_nextjs\_service\_jwt\_enabled) | Inject SERVICE\_JWT\_PRIVATE\_KEY into web-nextjs from Vault secret/dozuki/global/nextjs (Azure: KV secret 'nextjs') via ESO. That secret must exist in this env's backend first, or the dozuki-infra-credentials ExternalSecret stops syncing. Chart >= 1.8.0. | `bool` | `false` | no |
