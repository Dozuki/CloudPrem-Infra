...
# --- AWS Load Balancer Controller (self_managed) ---------------------------------
# Provides the TargetGroupBinding CRD + controller the logical layer uses to bind the
# physical NLB target groups to the Envoy Gateway pods. EKS Auto Mode bundles this; the
# self_managed path installs it (IAM + Pod Identity + helm via local-exec, like Karpenter).
