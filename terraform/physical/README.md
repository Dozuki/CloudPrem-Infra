...
| <a name="input_s3_kms_key_id"></a> [s3\_kms\_key\_id](#input\_s3\_kms\_key\_id) | KMS key identifier (key id, key ARN, alias name, or alias ARN) for the donor key that encrypted an existing set of Dozuki buckets being migrated in via s3\_existing\_buckets. Only used to grant this stack's roles decrypt access to that key; it never encrypts buckets this stack creates, which always use their own KMS key. | `string` | `""` | no |
