...
| <a name="input_eks_cluster_exists"></a> [eks\_cluster\_exists](#input\_eks\_cluster\_exists) | Set to true after the first successful apply. Enables reading the live cluster version to prevent drift from EKS auto-upgrades. | `bool` | `false` | no |
| <a name="input_eks_k8s_version"></a> [eks\_k8s\_version](#input\_eks\_k8s\_version) | Initial Kubernetes version for new clusters. Ignored after creation — EKS Auto Mode manages upgrades via upgrade\_policy. | `string` | `"1.33"` | no |
