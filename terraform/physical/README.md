...
| <a name="input_rds_engine_version"></a> [rds\_engine\_version](#input\_rds\_engine\_version) | To support legacy systems on mysql5.7 we allow setting the engine version here. Only 5.7 or 8.0 are allowed. | `string` | `"8.0"` | no |
...
| <a name="input_s3_block_public_access"></a> [s3\_block\_public\_access](#input\_s3\_block\_public\_access) | To comform with SCP we can disable adding a public access block to the S3 buckets. This should only be disabled if absolutely necessary. | `bool` | `true` | no |
