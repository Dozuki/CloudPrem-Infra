...
locals {
  ...
  vpc_id             = local.create_vpc ? try(module.vpc[0].vpc_id, "") : var.vpc_id
  vpc_cidr           = local.create_vpc ? try(module.vpc[0].vpc_cidr_block, "") : data.aws_vpc.this[0].cidr_block
  public_subnet_ids  = local.create_vpc ? try(module.vpc[0].public_subnets, "") : data.aws_subnets.public[0].ids
  private_subnet_ids = local.create_vpc ? try(module.vpc[0].private_subnets, "") : data.aws_subnets.private[0].ids
}
