...
| `dms_task_arn` / `dms_enabled` | BI replication. Serverless: a `replication-config` ARN, not a `task` ARN — the logical layer's dms-start Job branches on the ARN shape | `dms_task_arn` / `dms_enabled` | N/A — `""` / `false` (BI deferred past Azure v1) |
