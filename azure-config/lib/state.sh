...
ensure_state_backend() {
  ...
  _write_backend "${PHYSICAL_DIR}/backend_override.tf" "$rg" "$sa" "$container" "physical.tfstate"
  _write_backend "${LOGICAL_DIR}/backend_override.tf"  "$rg" "$sa" "$container" "logical.tfstate"
  log "remote state ready: ${sa}/${container}"
}

_write_backend() { # path rg sa container key
  cat > "$1" <<EOF
