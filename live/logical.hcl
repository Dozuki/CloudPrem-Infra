# Shared config for logical stacks (live harness): the dependency on the sibling
# physical stack + the logical inputs. Pairs with chart-helmignores.hcl.
dependency "physical" {
...
inputs = {
  ...
  dms_replication_generation    = try(dependency.physical.outputs.dms_replication_generation, "")
